...
def test_project_config_legacy_upgraded(tmpdir: TempDir) -> None:
    ...
    compare(
        ...
        expected=[
            Finding('project-config', f'{project}/config.yaml missing kind: project', True, True)
        ],
    )
...
def test_project_config_unexpected_kind_without_repo_not_fixable(tmpdir: TempDir) -> None:
    ...
    compare(
        ...
        expected=[
            Finding(
                'project-config', f'{project}/config.yaml has unexpected kind: bogus', False, False
            )
        ],
    )
...
def test_human_worktree_unmerged_left_in_place(tmpdir: TempDir) -> None:
    ...
    compare(
        ...
        expected=[
            Finding(
                'human-worktrees', f'{worktree} has unmerged commits — left in place', False, False
            )
        ],
    )
...
def test_legacy_separator_migrated(tmpdir: TempDir) -> None:
    ...
    compare(
        ...
        expected=[
            Finding(
                'worktree-separator', 'legacy worktree my-goal-agent → my-goal@agent', True, True
            )
        ],
    )
...
def test_legacy_separator_report_only_leaves_it(tmpdir: TempDir) -> None:
    ...
    compare(
        ...
        expected=[
            Finding(
                'worktree-separator', 'legacy worktree my-goal-agent → my-goal@agent', False, True
            )
        ],
    )
...
def test_orphaned_registration_report_only_keeps_it(tmpdir: TempDir) -> None:
    ...
    compare(
        registered_worktrees(Git(repo.path)), expected={repo.path.resolve(), worktree.resolve()}
    )
