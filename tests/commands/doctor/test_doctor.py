...
from chimera.commands.doctor import auth as doctor_auth
...
def _no_chimera_checkout(replace: Replacer) -> None:
    ...
    # and claude-hooks reads/writes the user's global ~/.claude/settings.json — likewise
    replace.in_module(
    ...
    # and harness-auth reads this machine's real keychain/credentials — likewise
    replace.in_module(doctor_auth.credential_states, lambda: {})
