...
- **claude-hooks** — chimera's session-capture + mail-delivery hooks are installed in the
  ...
  `ch hook deliver`). SessionStart also carries the occupancy warning — the exclusive-launch
  guard's reach into harness-native attaches/revives (see *Worktrees*, `agent start`/`resume`).
  `--fix` merges them in idempotently, preserving any existing hooks while
...
worktree at all (a typo must never read as "nothing running"). `goal merge` calls the same
...
`ch agent start` launches `claude` in an existing worktree (`--name <project>@<goal>@<actor>`); `ch agent resume` reattaches to the same session. Identity comes from the **archive**, not the name: the address `(project, goal, actor)` resolves to its newest archived session — live or dead — and resume reattaches by that immutable native id, re-asserting the canonical `--name` (registry names are mutable; a rename in claude's own UI must neither orphan the session nor survive the reattach). Only when the archive has never seen the address (or there's no workspace to hold one) does it fall back to `claude --resume <name>`. Liveness and pids stay with the registry (`agent stop` is keyed by worktree cwd, never name). Resume exists because `claude` has no `--cwd`: Chimera knows the worktree and sets it, so a dead session is revived in the right place from anywhere. Both run foreground, or background (`--bg`) when a `[prompt]` is given, and both refuse if a session is already live in the worktree — the exclusive-launch guard every goal-worktree launcher (`agent start`/`resume`, `goal start`/`adopt`, `review`) takes; only `ch chat` opts out, and it refuses goal scopes anyway. A harness-native attach or revive — the `claude agents` browser, a raw `claude --resume` run in the worktree — never reaches those launchers, so it can't be refused: the SessionStart hook warns instead. When a conversation starts or revives into a goal worktree another session is verified-live in, the hook lands a WARNING in the log (occupant ids bound) and injects a warning into the starting session's own context naming the occupants, so it surfaces the double-occupancy before doing any work — two writers on one branch is how commits appear that neither session made.
