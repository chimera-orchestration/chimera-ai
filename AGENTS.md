...
- **Seat** — the address a *place* speaks for, never a claim about who is standing in it: the workspace root is the captain's seat, a project dir its manager's, a goal worktree its agent's. It decides what you send mail as and whose inbox you read by default. A human standing at the workspace root shares the captain's seat — intended, and it still makes them nobody: a seat entitles no session to an **address**. Every log line carries both — `caller` (who ran it, on evidence) and `seat` (where from).
...
- **The machine never decides a test** – a test's outcome must come from what the test set up, never from what happens to be installed, configured or logged in on the box running it. The box we develop on is the least representative one there is — the suite runs *inside* claude, with your git identity and your `~/.claude`; CI is what an empty machine looks like, and it is the oracle. A new host dependency earns a positional guard in `tests/conftest.py`, never a stub in the one test file that noticed. More in @agent-docs/unit-and-functional-testing.md.
